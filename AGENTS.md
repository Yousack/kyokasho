...
- Prefer the established vocabulary of the mathematical subject over generic computational metaphors. Use terms such as domain, codomain, image, preimage, mapping, and action where they express the structure more precisely. Use "input" and "output" sparingly when first clarifying the direction of a rule or when a computational interpretation is itself relevant; do not repeat them as the default description of every map or equation.
- When introducing vectors geometrically, begin with points and displacement when that motivation is available. Distinguish a point, a displacement vector, and the position vector obtained after choosing an origin before identifying all of them with coordinate tuples.
